...
export default class WEB4DS {
  constructor(id, urlD, urlM, urlA, position, renderer, scene, camera) {
    ...
    this.framerate = 0 // call framerate for calculate the time
    ...
    if (this.waiterParent) {
      ...
      this.waiterElemLogo.src = '/static/web4dv/waiter/waiter.svg'
  ...
  initSequence(nbFrames, nbBlocs, framerate, maxVertices, maxTriangles, textureEncoding, textureSizeX, textureSizeY, modelPosition) {
    ...
    this.scene.add(this.model4D.ambLight)
    this.scene.add(this.model4D.light1)
    this.scene.add(this.model4D.light2)
    this.scene.add(this.model4D.light3)
